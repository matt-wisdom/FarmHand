...
def get_embedding_model() -> Any:
    ...
    if _embedding_model is None:
        ...
        try:
            ...
            print(
                f"[rag_pipeline] FastEmbed offline load fallback activated ({e}). Using OfflineVectorEmbedder."
            )
...
def get_faiss_index() -> faiss.Index | None:
    global _faiss_index
...
def search_knowledge_base(search_query: str, top_k: int = 3) -> list[dict[str, Any]]:
    ...
    """
    print(
        f"[rag_pipeline] search_knowledge_base query: '{search_query}' top_k: {top_k}"
    )

    faiss_idx = get_faiss_index()
    print(
        f"[rag_pipeline] FAISS index total: {faiss_idx.ntotal if faiss_idx else 'None'}"
    )
    ...
    # Get BM25 results
    print("[rag_pipeline] Running BM25 search...")
    bm25_results = bm25_search(search_query, top_k=top_k * 3)
    print(f"[rag_pipeline] BM25 results count: {len(bm25_results)}")
    if bm25_results:
        print(
            f"[rag_pipeline] BM25 top result: {bm25_results[0].get('text', '')[:100]}..."
        )

    # Get vector results
    print("[rag_pipeline] Running vector search...")
    vector_results = vector_search(search_query, top_k=top_k * 3)
    print(f"[rag_pipeline] Vector results count: {len(vector_results)}")
    if vector_results:
        print(
            f"[rag_pipeline] Vector top result: {vector_results[0].get('text', '')[:100]}..."
        )

    # Combine with weighted scoring
    print("[rag_pipeline] Combining results...")
...
def vector_search(query: str, top_k: int = 10) -> list[dict[str, Any]]:
    ...
    if len(indices) > 0 and len(indices[0]) > 0:
        hit_ids = [
            int(i.item()) if hasattr(i, "item") else int(i)
            for i in indices[0]
            if i >= 0
        ]
        if not hit_ids:
        ...
        for dist, faiss_id in zip(distances[0], hit_ids, strict=False):
            db_id = faiss_id + 1
            if db_id in rows:
                item = rows[db_id]
...
def get_bm25_retriever() -> Any:
    ...
    if _bm25_retriever is None:
        ...
        if save_path.exists():
            ...
            print("[rag_pipeline] BM25 index not found. Run build_bm25_index.py first.")
            return None
...
def bm25_search(query: str, top_k: int = 10) -> list[dict[str, Any]]:
    ...
    try:
        ...
        sqlite_ids = [
            int(idx.item()) + 1 if hasattr(idx, "item") else int(idx) + 1
        ...
        final_results = []
        for rank, (idx, score) in enumerate(zip(doc_indices, scores, strict=False)):
...
def combine_results(
    bm25_results: list[dict], vector_results: list[dict], top_k: int
) -> list[dict]:
    ...
    sorted_results = sorted(
        score_map.values(), key=lambda x: x["combined"], reverse=True
    )
...
def add_document_to_knowledge_base(farm_id: str, pdf_path: Path) -> dict[str, Any]:
    ...
    try:
        # Add to vector store
        chunks_added = add_document_to_vector_store(pdf_path=pdf_path, farm_id=farm_id)
...
def delete_farm_document(farm_id: str, filename: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {"success": True, "filename": filename, "chunks_removed": chunks_removed}
    except Exception as e:
        return {"success": False, "error": str(e)}
