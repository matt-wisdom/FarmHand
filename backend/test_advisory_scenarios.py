...
res1 = chat_completion(
    [{"role": "user", "content": q1}], farm_id="default_farm", language="english"
)
t1 = time.time()
print(f"Done in {t1 - t0:.2f}s:\n{res1}\n")

assert "newcastle" not in res1.lower(), (
    "Should NOT hallucinate Newcastle Disease for flock integration"
)
assert any(
    k in res1.lower() for k in ["quarantine", "isolate", "separate", "visual", "pen"]
), "Should mention practical integration steps"
assert not res1.startswith("["), "Must not be JSON"
assert (
    " dey " not in res1.lower()
    and "na " not in res1.lower()
    and "wetin" not in res1.lower()
), "Must not contain Pidgin"
...
res2 = chat_completion(
    [{"role": "user", "content": q2}], farm_id="default_farm", language="english"
)
t1 = time.time()
print(f"Done in {t1 - t0:.2f}s:\n{res2}\n")
...
assert (
    " dey " not in res2.lower()
    and "na " not in res2.lower()
    and "wetin" not in res2.lower()
), "Must not contain Pidgin"
...
assert (
    " dey " not in res3.lower()
    and "na " not in res3.lower()
    and "wetin" not in res3.lower()
), "Must not contain Pidgin"
