...
from tool_registry import match_species  # noqa: E402
...
def test_inventory_query_routing():
    ...
    thread_id = database.create_chat_thread(
        title="Inventory Test", farm_id="default_farm"
    )
    ...
    response = chat_completion(
        messages, farm_id="default_farm", thread_id=thread_id, language="english"
    )
    print(f"\n[Prompt]: {prompt}\n[Response]:\n{response}\n")
    ...
    assert (
        "0" in response
        or "zero" in response.lower()
        or "no" in response.lower()
        or "none" in response.lower()
    )
...
def test_registered_animals_inventory():
    ...
    thread_id = database.create_chat_thread(
        title="Active Animals Test", farm_id="default_farm"
    )
    ...
    with database.get_db_connection() as conn:
        conn.cursor().execute(
            "DELETE FROM animals WHERE id IN ('CHICK-001', 'CHICK-002', 'GOAT-001')"
        )
...
def test_rag_clinical_query_routing():
    ...
    thread_id = database.create_chat_thread(
        title="Clinical RAG Test", farm_id="default_farm"
    )
