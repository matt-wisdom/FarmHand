...
_ha_en_tok: Optional[MarianTokenizer] = None
_ha_en_model: Optional[MarianMTModel] = None

_en_ha_tok: Optional[MarianTokenizer] = None
_en_ha_model: Optional[MarianMTModel] = None


def get_ha_en_translator():
    global _ha_en_tok, _ha_en_model
    if _ha_en_model is None or _ha_en_tok is None:
        model_name_or_path = str(HA_EN_DIR) if HA_EN_DIR.exists() else "Helsinki-NLP/opus-mt-ha-en"
        local_only = HA_EN_DIR.exists()
        print(f"[translator] Loading ha->en model from {model_name_or_path} (local_only={local_only})...")
        _ha_en_tok = MarianTokenizer.from_pretrained(model_name_or_path, local_files_only=local_only)
        _ha_en_model = MarianMTModel.from_pretrained(model_name_or_path, local_files_only=local_only)
        _ha_en_model.eval()
...
def get_en_ha_translator():
    ...
    if _en_ha_model is None or _en_ha_tok is None:
        model_name_or_path = str(EN_HA_DIR) if EN_HA_DIR.exists() else "Helsinki-NLP/opus-mt-en-ha"
        local_only = EN_HA_DIR.exists()
        print(f"[translator] Loading en->ha model from {model_name_or_path} (local_only={local_only})...")
        _en_ha_tok = MarianTokenizer.from_pretrained(model_name_or_path, local_files_only=local_only)
        _en_ha_model = MarianMTModel.from_pretrained(model_name_or_path, local_files_only=local_only)
        _en_ha_model.eval()
...
def translate_ha_to_en(text: str) -> str:
    ...
    try:
        ...
        prefix = ""
        if any(w in lower_raw for w in ["kaji", "kaza", "tsuntsaye", "dakin kaji"]) and not any(w in translated.lower() for w in ["chicken", "poultry", "bird", "hen", "coop"]):
            prefix += "poultry chickens "
        if any(w in lower_raw for w in ["awaki", "akuya"]) and not any(w in translated.lower() for w in ["goat", "buck", "doe"]):
            prefix += "goats "
        if any(w in lower_raw for w in ["shanu", "saniya"]) and not any(w in translated.lower() for w in ["cattle", "cow", "bull"]):
            prefix += "cattle cows "
        if any(w in lower_raw for w in ["tumaki", "rago"]) and not any(w in translated.lower() for w in ["sheep", "ram"]):
            prefix += "sheep "
    ...
    try:
        ...
        for line in lines:
            if not line.strip():
                ...
                continue
            inputs = tok(line, return_tensors="pt", padding=True, truncation=True, max_length=512)
            with torch.no_grad():
                translated_tokens = model.generate(**inputs, max_length=512)
            translated = tok.decode(translated_tokens[0], skip_special_tokens=True)
